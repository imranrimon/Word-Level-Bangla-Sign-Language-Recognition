...
class tattn_block(nn.Module):
    def __init__(self, in_channels, out_channels, t_num_heads=4, kernel_size=1, stride=1, num_point=25, block_size=41, depth=1, wss=6, sr_ratios=8, use_ttsa=True, **kwargs):
        ...
        if self.use_ttsa:
            ...
            # Ablation: w/o TTSA (Use standard MHSA instead of TwinSVT)
            # TwinSVT input: (B, T, C). output: (B, C, T, 1) or similar.
            # Code below expects: x = self.attn(x, T, 1).flatten(-2)
            # Standard MHSA expects: (B, N, C) -> (B, N, C)
            self.attn = MHSA(dim=h_dim, out_dim=h_dim, num_heads=t_num_heads, qkv_bias=True)
    ...
    def forward(self, x, keep_prob, A):
        ...
        if self.use_ttsa:
             ...
             x = self.attn(x) # MHSA over T dimension
             x = x.transpose(1, 2) # (BV, T, C) -> (BV, C, T)
...
class SATTN_TCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, block_size, stride=1, residual=True, use_grpe=True, use_paf=True, use_ttsa=True, s_pos_emb=False, s_num_heads=1, t_pos_emb=False, t_num_heads=4, is_first=False, t_depth=1, **kwargs):
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, groups=8, block_size=41, graph=None, graph_args=dict(), in_channels=3, use_grpe=True, use_paf=True, use_ttsa=True, inner_dim=64, drop_layers=3, depth=4, s_num_heads=1, t_num_heads=8, window_size=120, s_pos_emb=True, t_pos_emb=True, t_depths=[2, 2, 2, 2], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1]):
