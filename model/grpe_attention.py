...
class unit_sattn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, num_subset=3, s_pos_emb=False, s_num_heads=1, t_pos_emb=False, use_paf=True, **kwargs):
    ...
    def forward(self, x0):
        ...
        if self.use_paf:
            ...
            # Ablation: w/o Post-Attention Factor
            # x is (N, 3, C/3, T, V). Sum over subset dimension to aggregate.
            x = x.sum(dim=1) # (N, kc//3, T, V) -> (N, 64, T, V)
