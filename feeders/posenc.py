...
def scatter_add(src, index, dim=0, dim_size=None):
    ...
    # Create output tensor of size dim_size along dim
    out_shape = list(src.shape)
    out_shape[dim] = dim_size
    
    out = torch.zeros(out_shape, dtype=src.dtype, device=src.device)
    
    # Generate index with same shape as src
    if index.dim() != src.dim():
        # Broadcasting index to src shape needed for scatter_add
        # Assuming index is 1D or same dim but smaller?
        # In this specific file usage: 
        # deg = scatter_add(edge_weight, source, dim=0, dim_size=num_nodes)
        # edge_weight: (E), source: (E)
        # So dimensions match.
        pass
